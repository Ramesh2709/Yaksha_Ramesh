...
public class ControlSelectors extends BaseSetup {
	...
	public void sendTextByXpathinco(String xpaths, String value) throws InterruptedException {
		// waitForElementByxpath(xpaths);
		for (int i = 1; i <= 5; i++) {
			try {
				WebDriverWait wait = new WebDriverWait(incognitodriver, Duration.ofSeconds(5));
				wait.until(ExpectedConditions.elementToBeClickable(By.xpath(xpaths)));
				incognitodriver.findElement(By.xpath(xpaths)).sendKeys(value);
				break;
			} catch (UnhandledAlertException e) {
				// alertBox(true);
				Thread.sleep(500);
	...
	public void clickByXpathincog(String xpaths) throws InterruptedException {
		for (int i = 1; i <= 5; i++) {
			try {
				WebDriverWait wait = new WebDriverWait(incognitodriver, Duration.ofSeconds(5));
				wait.until(ExpectedConditions.elementToBeClickable(By.xpath(xpaths)));
				incognitodriver.findElement(By.xpath(xpaths)).click();
				break;
			} catch (UnhandledAlertException e) {
				// alertBox(true);
				Thread.sleep(500);
	...
	public void clickByXpathpagedown(String xpaths) throws InterruptedException {
		for (int i = 1; i <= 5; i++) {
			try {
				WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(5));
				wait.until(ExpectedConditions.elementToBeClickable(By.xpath(xpaths)));
	...
	public boolean checkelementincog(String xpaths) {
		WebElement e=incognitodriver.findElement(By.xpath(xpaths));
			if (e.isDisplayed())
				return true;
			else
			return false;
